...
int main(int argc, char **argv) {
  ...
  /*for (u32 j = 0; j < 32; j ++) {
    for (u32 i = 0; i < 64; i ++) {
      ctx->framebuffer[j * 64 + i] = 1;
    }
  }
  for (u32 j_ = 0; j_ < 4; j_ ++) {
    for (u32 i_ = 0; i_ < 8; i_ ++) {
      u32 j = j_*(32 / 4) + 1;
      u32 i = i_*(64 / 8) + 1;
      ctx->framebuffer[(j+1) * 64 + (i+1)] = 0;
      ctx->framebuffer[(j+1) * 64 + (i+4)] = 0;
      ctx->framebuffer[(j+3) * 64 + (i+1)] = 0;
      ctx->framebuffer[(j+4) * 64 + (i+1)] = 0;
      ctx->framebuffer[(j+4) * 64 + (i+2)] = 0;
      ctx->framebuffer[(j+4) * 64 + (i+3)] = 0;
      ctx->framebuffer[(j+4) * 64 + (i+4)] = 0;
      ctx->framebuffer[(j+3) * 64 + (i+4)] = 0;
    }
  }
  */
